...
def upload_image(context, timeout_secs, image_service, image_id, owner_id,
    ...
    tmp_file_name = "/tmp/" + str(uuid.uuid4()) + ".vmdk"
    fp = open(tmp_file_name, "wb")
    start_transfer(context,timeout_secs, read_handle, file_size,
                   write_file_handle=fp)
    ...
    converted_file_name = "/tmp/" + str(uuid.uuid4()) + ".qcow2"
    convert_command="qemu-img convert -f vmdk -O qcow2 " + tmp_file_name  + " " + converted_file_name
    result = subprocess.call([convert_command],shell=True)
    if result == 0:
    ...
    image_metadata = {'disk_format': 'qcow2',
                      ...
                      'properties': {'vmware_image_version':
                                     ...
                                     'owner_id': owner_id}}
    read_file_handle_local = rw_util.HybridFileHandle(upload_file_name, "rb")
