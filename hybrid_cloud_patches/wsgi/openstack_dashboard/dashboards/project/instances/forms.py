...
class MigrateInstanceForm(forms.SelfHandlingForm):
    instance_id = forms.CharField(widget=forms.HiddenInput())
    az = forms.ChoiceField(label=_("Availability Zone"))

    def __init__(self, request, *args, **kwargs):
        super(MigrateInstanceForm, self).__init__(request, *args, **kwargs)
        ...
        self.fields['az'].choices = zone_list

    def clean(self):
        return super(MigrateInstanceForm, self).clean()
    ...
    def handle(self, request, data):
        ...
        az = data.get('az')
        try:
            print instance + " to " + az
            api.nova.server_migrate(request, instance, az)
            messages.success(request, _('Migrating instance %s.') % instance)
        except Exception:
            redirect = reverse('horizon:project:instances:index')
            exceptions.handle(request, _("Unable to migrate instance."), redirect=redirect)
        return True
