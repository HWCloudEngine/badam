...
class API(base.Base):
    ...
    def _validate_and_build_base_options(self, context, instance_type,
        ...
        if availability_zone:
            ...
            if forced_host is not None:
                try:
                    service = db.service_get_by_compute_host(context.elevated(), forced_host)
                except exception.NotFound:
                    msg = _('the host is not found')
                    raise exception.InvalidRequest(msg)
                zones = availability_zones.get_host_availability_zone(
                    context.elevated(),forced_host)
                if availability_zone not in zones:
                    msg = _('The requested host is not in the requested zones.')
                    raise exception.InvalidRequest(msg)
        ...
        for bdm in block_device_mapping:
            ...
            if bdm.get('image_id'):
                ...
                if volume['shareable'] == 'true':
                    raise exception.InvalidShareableBDMVolumeForBoot(
                            id=volume_id)
    ...
    def _create_instance(self, context, instance_type,
        ...
        availability_zone, forced_host, forced_node = handle_az(context,
                                                            availability_zone)
        if str(availability_zone) is '':
            availability_zone = None
        if str(forced_host) is '':
            forced_host = None
        ...
        # NOTE(lee) this change is to persist requested_networks
        # info, HA function will use it.
        req_update_networks = obj_base.obj_to_primitive(requested_networks or [])
        update_networks = []
        for req_update_network in req_update_networks:
            update_network = [req_update_network.get("network_id"),
                              req_update_network.get("address"),
                              req_update_network.get("port_id"),
                              req_update_network.get("pci_request_id")]
            update_networks.append(update_network)

        for instance in instances:
            try:
                objects.HuaweiInstanceExtra(
                    instance_uuid=instance.uuid,
                    request_network=jsonutils.dumps(
                        update_networks),
                    scheduler_hints=jsonutils.dumps(
                        scheduler_hints),
                    injected_files=jsonutils.dumps(
                        injected_files or [])).create(context)
            except:
                LOG.debug("requested_networks info update fail")
                pass
    ...
    def snapshot_volume_backed(self, context, instance, image_meta, name,
        ...
        for bdm in bdms:
            ...
            if bdm.is_volume:
                ...
                volume = self.volume_api.get(context, bdm.volume_id)                
                if volume['shareable'] == 'true':
                    LOG.debug(_("skip shareable volume %(volume_id)s"
                                " when do snapshot"),
                              {'volume_id': bdm.volume_id}, instance=instance)
                    continue
    ...
    def resize(self, context, instance, flavor_id=None,
        ...
        migrate_host = extra_instance_updates.pop('migrate_host', None)
        if migrate_host:
            filter_properties['migrate_host'] = migrate_host
    ...
    def swap_volume(self, context, instance, old_volume, new_volume):
        ...
        if old_volume['instance_uuid'] != instance['uuid']:
            ...
            raise exception.InvalidVolume(reason=msg)
        if new_volume['shareable'] == 'false' and \
                new_volume['attach_status'] == 'attached':
