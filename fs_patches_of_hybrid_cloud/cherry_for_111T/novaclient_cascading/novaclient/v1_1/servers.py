...
class Server(base.Resource):
    ...
    def migrate(self, az=None, no_sys_vol=False):
        """
        Migrate a server to a new host.
        """
        self.manager.migrate(self, az, no_sys_vol)
...
class ServerManager(base.BootingManagerWithFind):
    ...
    def migrate(self, server, az=None, no_sys_vol=False):
        """
        Migrate a server to a new host.

        :param server: The :class:`Server` (or its ID).
        """
        self._action('migrate', server, {'az':az, 'no_sys_vol':no_sys_vol})
