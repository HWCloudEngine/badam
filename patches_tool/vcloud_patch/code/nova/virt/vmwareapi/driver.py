    ...
    def reconfigure_affinity_group(self, instances, affinity_group, type,
        ...
        if instances != []:
            if node != None:
                ...
                node_temp = instances[0]['node']
            for instance in instances:
                if node_temp != instance['node']:
                    raise error_util.VMwareAffinityGroupException()
        _vmops = self._get_vmops_for_compute_node(node_temp)
        _vmops.reconfigure_affinity_group(instances, affinity_group, type)

    def update_vif_pg_info(self, instance, network_info):
        """update vif port group in vmware."""
        _vmops = self._get_vmops_for_compute_node(instance['node'])
        _vmops.update_vif_pg_info(instance, network_info)
...
class VMwareAPISession(api.VMwareAPISession):
    ...
    def _get_vim(self):
        ...
        if not self._plugins:
            vim.client.set_options(plugins=[VMwareServiceMessagePlugin()])
            plugins = suds.plugin.PluginContainer(vim.client.options.plugins)
            plugins.init.initialized(wsdl=vim.client.wsdl)
            self._plugins = vim.client.options.plugins
        return vim
...
class VMwareServiceMessagePlugin(service.ServiceMessagePlugin):
