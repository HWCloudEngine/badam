...
class ComputeManager(manager.Manager):
    ...
    def _get_csd_instance_cache(self, instance):
        ...
        proxy_instance_uuid = (self._uuid_mapping.get(instance_uuid, {})
                               .get('mapping_id', None))
        if not proxy_instance_uuid:
            ...
            try:
                ...
                if vms:
                    self._uuid_mapping[instance_uuid] = \
                        {'mapping_id': vms[0].id,
    ...
    def _delete_proxy_instance(self, context, instance, delete_in_db=True):
        ...
        ret = dict(success=False, retry=0, retry_max=30, loop_time=5)
        ...
        def _wait_for_delete_instance(context, csd_uuid):
            ...
            try:
                ...
                return
            if csd_server:
                LOG.debug('csd_server:%s status:%s', csd_uuid, csd_server._info['OS-EXT-STS:vm_state'])
                return
    ...
    def _heal_syn_flavor_info(self, context, instance_type):
        ...
        def syn_flavor_info(context, instance_type):
            ...
            if csd_instance_type.get('extra_specs', {}) != instance_type.get('extra_specs', {}):
                csd_flavor.unset_keys(csd_flavor.get('extra_specs', {}))
                csd_flavor.set_keys(instance_type.get('extra_specs', {}))
    ...
    def _heal_instance_state(self, context):
        ...
        try:
            ...
            _change_since_time = (timeutils.utcnow() - datetime.timedelta(
                seconds=time_shift_tolerance))
            ...
            while True:
                ...
                for server in servers:
                    ...
                    if csg_uuid:
                        ...
                        csd_vm_state = server._info['OS-EXT-STS:vm_state']
                        csd_task_state = server._info['OS-EXT-STS:task_state']
                        if (instance.vm_state == vm_states.RESIZED or
                                    instance.task_state in EXCLUDE_REFRESH_TASK_STATES) and \
                                        csd_vm_state != vm_states.ERROR:
                            continue
                        if csd_vm_state == vm_states.DELETED:
                            continue
                        mapping = self._get_csd_instance_cache(instance)
                        server_host = server.hostId
                        server_node = server._info['OS-EXT-SRV-ATTR:hypervisor_hostname']
                        if mapping and (
                                    mapping.get('csd_host_id',
                                                None) != server_host or
                                    mapping.get('csd_node_id',
                                                None) != server_node):
                            self._heal_host_change_event(context, csg_uuid, server_host, server_node)
                            mapping['csd_host_id'] = server_host
                            mapping['csd_node_id'] = server_node

                        if csd_vm_state == vm_states.ERROR:
                            if not instance['fault']:
                                fault = server._info.get('fault')
                                self._create_instance_fault(context, instance, fault)

                        # Refresh only the final state
                        elif csd_task_state:
                            continue
                        elif csd_vm_state in EXCLUDE_REFRESH_VM_STATES:
                            continue
                        _msg = _('Updated the servers:%s'
                                 'vm_state:%s task_state:%s power_state:%s') \
                               % (csg_uuid, csd_vm_state, csd_task_state, server._info['OS-EXT-STS:power_state'])
                        LOG.debug(_msg)
                        self._instance_update(
                            context,
                            csg_uuid,
                            vm_state=(
                            vm_states.ERROR if instance.vm_state == vm_states.ERROR else csd_vm_state),
            ...
            LOG.error(_('Failed to sys server status to db. %s'), msg)
    ...
    def _create_instance_fault(self, context, instance, fault):
        ...
        fault_obj.code = fault['code']
        fault_obj.details = fault.get('details', None)
        fault_obj.message = fault.get('message', None)
        fault_obj.create()


    def _heal_host_change_event(self, context, instance_uuid, server_host, server_node):
        try:
        ...
        try:
            if CONF.resource_tracker_synced:
                instance.host = CONF.proxy.host_sync_prefix + "#" + server_host
                instance.node = server_node
            else:
        ...
        if (instance.vm_state == vm_states.BUILDING and
            ...
            try:
                ...
                if not vms:
                    ...
                    server = vms[0]
                    instance.vm_state = server._info['OS-EXT-STS:vm_state']
                    instance.task_state = server._info['OS-EXT-STS:task_state']
                    instance.power_state = server._info['OS-EXT-STS:power_state']
                    instance.launched_on = server._info['OS-SRV-USG:launched_at']
    ...
    def _init_error_instance(self, instance):
        ...
        try:
            vms = self.sync_nova_client.servers.list(search_opts=search_opts)
            if vms and vms[0]._info['OS-EXT-STS:vm_state'] != vm_states.ERROR:
                server = vms[0]
                instance.power_state = server._info['OS-EXT-STS:power_state']
                instance.launched_on = server._info['OS-SRV-USG:launched_at']
                LOG.warn(_('Instance:%s vm_states ERROR,update power_state:%s from csd'),
                         instance.uuid,
                         instance.power_state)
                instance.save()
        except novaclient.exceptions.NotFound as e:
    ...
    def _init_instance(self):
        ...
        try:
            ...
            error_instances = self.query_instances(filters=filters)
            LOG.debug(_('error instances size:%s'), len(error_instances))
            for instance in error_instances:
                self._init_error_instance(instance)
    ...
    def _migrate_cross_cloud(self, context, instance, flavor_prefix=''):
        ...
        request_spec['instance_type'] = flavors.extract_flavor(instance, prefix=flavor_prefix)
        ...
        self._wait_for_cascaded_instance(context, instance, vm_states.ACTIVE)

    def _wait_for_cascaded_instance(self, context, instance, state):
        LOG.info(_LI("Wait for instance change to status %s."), state)
        cascaded_instance_id = self._get_csd_instance_uuid(instance)
        if cascaded_instance_id:
            cascaded_nova_cli = self._get_nova_python_client(context)
            ...
            def _wait_for_running():
                """Called at an interval until the VM is running."""
                if _get_cascaded_instance_status() == state:
                    LOG.info(_LI("Instance spawned successfully."),
                             instance=instance)
                    raise loopingcall.LoopingCallDone()
    ...
    def _attach_volume(self, context, instance, bdm):
        ...
        try:
            ...
            cascaded_nova_cli.volumes.create_server_volume(
                proxy_instance_id,proxy_volume_id, bdm['mount_device'])
    ...
    def detach_interface(self, context, instance, port_id):
        ...
        detach_data = dict(success=False, retry=0, retry_max=30, loop_time=5)
        def _ensure_detach():
            if detach_data['retry'] > detach_data['retry_max']:
    ...
    def _update_host_aggregate(self, context):
        ...
        LOG.debug('csd_host_aggregates: %s', csd_host_aggregates)
        LOG.debug('csg_host_aggregates: %s', csg_host_aggregates)
